...
const updateUserInterests = (body, currentInterests = {}) => {
  ...
  if (interestTopics) {
    result.topics = Array.isArray(interestTopics)
      ? interestTopics
      : interestTopics.split(",").map((item) => item.trim());
  }

  if (interestIndustries) {
    result.industries = Array.isArray(interestIndustries)
      ? interestIndustries
      : interestIndustries.split(",").map((item) => item.trim());
  }
...
exports.getCurrentUser = async (req, res) => {
  try {
    const user = await User.findById(req.user.id)
      .select(
        "-password -security.passwordResetToken -security.passwordResetExpires"
      )
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    if (education) {
      ...
      profileFields.education = Array.isArray(education)
        ? education
        : [education];
    }
    ...
    if (experience) {
      ...
      profileFields.experience = Array.isArray(experience)
        ? experience
        : [experience];
    }
  ...
  try {
    ...
    if (
      ...
      !user.connections.some(
        (connection) => connection._id.toString() === req.user.id
      ) &&
...
exports.recordProfileView = async (req, res) => {
  try {
    ...
    if (req.user.id === userId) {
      return res
        .status(400)
        .json({ error: "Cannot record view for your own profile" });
    }
...
exports.getProfileViewers = async (req, res) => {
  try {
    ...
    profileViews.forEach((view) => {
      ...
      if (
        !viewersMap.has(viewerId) ||
        viewersMap.get(viewerId).timestamp < view.timestamp
      ) {
...
exports.updateProfileViewPrivacy = async (req, res) => {
  try {
    const { viewsVisibility, allowAnonymousViews, notifyOnProfileView } =
      req.body;
...
exports.connectCalendar = async (req, res) => {
  try {
    ...
    if (!provider || !accessToken) {
      return res
        .status(400)
        .json({ error: "Provider and access token are required" });
    }
...
exports.getCalendarEvents = async (req, res) => {
  try {
    ...
    if (
      !user.integrations ||
      !user.integrations.calendar ||
      !user.integrations.calendar.connected
    ) {
...
exports.connectSocialAccount = async (req, res) => {
  try {
    const { provider, accessToken, refreshToken, expiresAt, profile } =
      req.body;
    ...
    const existingIndex = user.integrations.social.findIndex(
...
exports.disconnectSocialAccount = async (req, res) => {
  try {
    ...
    const providerIndex = user.integrations.social.findIndex(
      (s) => s.provider === provider
    );
...
exports.getUserStats = async (req, res) => {
  try {
    ...
    console.error(
      `Error fetching user stats for ID ${req.params.userId}:`,
      error
    );
    res.status(500).json({ error: "Server error" });
  }
...
exports.getUserEducation = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).select("education").lean();
    ...
    console.error(
      `Error fetching education for user ${req.params.userId}:`,
      error
    );
    res.status(500).json({ error: "Server error" });
  }
...
exports.getSocialAccounts = async (req, res) => {
  try {
    ...
    const socialAccounts =
      user.integrations && user.integrations.social
...
exports.shareToSocial = async (req, res) => {
  try {
    ...
    if (!provider || !contentType || !contentId) {
      return res
        .status(400)
        .json({ error: "Provider, content type, and content ID are required" });
    }
    ...
    if (
      ...
      !user.integrations.social.some(
        (s) => s.provider === provider && s.connected
      )
    ) {
      return res
        .status(400)
        .json({ error: `${provider} account not connected` });
    }

    // Get the social account
    const socialAccount = user.integrations.social.find(
      (s) => s.provider === provider && s.connected
    );
...
exports.addNotificationToken = async (req, res) => {
  try {
    ...
    if (!token || !deviceType) {
      return res
        .status(400)
        .json({ error: "Token and device type are required" });
    }
